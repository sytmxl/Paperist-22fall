<template>
  <div class="main">
    <h1>待审核学者身份认领请求</h1>
    <el-table :data="files">
      <el-table-column fixed prop="time" label="提交时间" width="160">
      </el-table-column>
      <el-table-column prop="" label="用户id" width="120">
      </el-table-column>
      <el-table-column prop="author_id" label="申请认领的学者id"> </el-table-column>
    ...
    init(){
      this.$axios({
        ...
        for(var i=0;i<this.files.length;i++){
          this.files[i].time = this.files[i].time.split('T')[0]+' '+this.files[i].time.split('\.'|'T')[1]
        }
      });
    },
  },
