 ...
 this.$message.error("抱歉，复制失败！")
     ...
     get_authors(){
      ...
      axios({
        ...
        let authors = res.data.hits.hits
        let obj2 = {
        query:{
          bool:{
          must:[],
      ...
      obj2.query.bool.must.push({"match_phrase":{"authors.id":authors.id}})
      obj2.query.bool.filter={"match_phrase":{"authors.id":authors.id}}
      axios({
            headers: {
              'content-type': 'application/json',
            },
    ...
    //     }
    //   }
    //   obj.query.bool.must.push({"match_phrase":{"authors.id":}})
    //   obj.query.bool.filter={"match_phrase":{"authors.id":}}
    //   axios({
    //         headers: {
    //           'content-type': 'application/json',
    //         },
