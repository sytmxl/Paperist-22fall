...
export default {
  ...
  methods: {
    ...
    load_content(recommends,index){
      ...
      if(recommends[index].id!=null){
        obj.query.bool.must.push({"match_phrase":{"id":recommends[index]._source.id}})
      obj.query.bool.filter={"match_phrase":{"id":recommends[index]._source.id}}
      axios({
            headers: {
              'content-type': 'application/json',
            },
        ...
        if(index==0){
          this.recommend_content=res.data.hits.hits
        }
        else{
          // console.log(res.data.hits.hits)
          for(var i=0;i<res.data.hits.hits.length;i++){
            this.recommend_content.push(res.data.hits.hits[i])
          }
